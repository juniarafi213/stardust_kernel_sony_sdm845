...
static int release_sem_state(struct ntsync_obj *sem, __u32 count)
{
	...
	if (check_add_overflow_u32(sem->u.sem.count, count, &sum) ||
	    sum > sem->u.sem.max)
		return -EOVERFLOW;
...
{
	return ntsync_obj_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
}

static long ntsync_char_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	return ntsync_char_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
}
#else
#define ntsync_obj_compat_ioctl NULL
#define ntsync_char_compat_ioctl NULL
#endif
...
static const struct file_operations ntsync_obj_fops = {
	...
	.compat_ioctl 	= ntsync_obj_compat_ioctl,
	.llseek		= no_llseek,
};
...
static const struct file_operations ntsync_fops = {
	...
	.compat_ioctl 	= ntsync_char_compat_ioctl,
	.llseek		= no_llseek,
};
